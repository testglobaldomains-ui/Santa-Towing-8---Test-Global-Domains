  ...
  return (
    <main className="min-h-screen bg-slate-50 pt-24 pb-20">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        {/* Hero Header Section */}
        <div className="bg-white rounded-[1px] p-6 sm:p-10 border border-slate-200/80 shadow-sm mb-8 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-96 h-96 bg-accent/5 rounded-full blur-3xl -z-0 pointer-events-none" />
          
          <div className="relative z-10">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-bold uppercase tracking-wider bg-accent/10 text-accent rounded-[50px]">
                <Sparkles className="w-3.5 h-3.5" />
                Numbered Website Hierarchy
              </span>
              <span className="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold bg-slate-900 text-white">
                <Hash className="w-3 h-3 text-accent" />
                {totalLinksCount} Sequentially Numbered Pages
              </span>
              <span className="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold bg-slate-100 text-slate-700">
                <Layers className="w-3.5 h-3.5 text-slate-500" />
                11 Categories (A – K)
              </span>
              <span className="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 shrink-0">
                <a 
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold bg-slate-900 text-white hover:bg-accent transition-colors shadow-sm rounded-[50px]"
                ...
                <Link
                  to="/booking"
                  className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold bg-accent text-white hover:bg-accent/90 transition-colors shadow-sm shadow-accent/20 rounded-[50px]"
                >
                ...
                {SITEMAP_CATEGORIES.map(cat => {
                  ...
                  return (
                    <button className="rounded-[50px]"
                      ...
                      className="p-2 rounded-[1px] bg-slate-50 hover:bg-accent/10 hover:border-accent/30 border border-slate-200/80 text-left transition-all group rounded-[50px]"
                    >
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 flex flex-col md:flex-row items-center gap-4">
              <div className="relative w-full md:flex-1">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-10 py-3 bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 text-slate-900 placeholder:text-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all rounded-[50px]"
                />
                {searchQuery && (
                  <button className="rounded-[50px]"
              ...
              <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end">
                {/* View Mode Toggle */}
                <div className="inline-flex rounded-[1px] border border-slate-200 bg-slate-100 p-1">
                  <button className="rounded-[50px]"
                ...
                <button
                  onClick={expandAll}
                  className="text-xs font-semibold text-slate-600 hover:text-accent px-3 py-2 bg-slate-100 hover:bg-slate-200/70 transition-colors rounded-[50px]"
                >
                  Expand All
                </button>
                <button
        ...
        {searchQuery && (
          <div className="mb-6 px-2 flex items-center justify-between text-sm text-slate-600 bg-white p-4 rounded-[1px] border border-slate-200">
            <p>
        ...
        {filteredCategories.length === 0 ? (
          <div className="bg-white rounded-[1px] p-12 text-center border border-slate-200">
            ...
            <button className="rounded-[50px]"
              ...
              className="px-5 py-2.5 bg-accent text-white font-bold text-sm hover:bg-accent/90 transition-colors rounded-[50px]"
            >
            ...
            {filteredCategories.map((category) => {
              ...
              return (
                <section
                  key={category.id}
                  id={category.id}
                  className="bg-white rounded-[1px] sm:rounded-[1px] border border-slate-200/90 shadow-sm overflow-hidden transition-all duration-200"
                >
                  ...
                  >
                    <div className="flex items-center gap-3.5 sm:gap-4 min-w-0">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-[#192C2C] text-white flex items-center justify-center shrink-0 shadow-sm group-hover:bg-white group-hover:text-[#192C2C] transition-colors">
                        <IconComponent className="w-5 h-5 sm:w-6 sm:h-6" />
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          {/* Category Tag (A, B, C...) */}
                          <span className="inline-flex items-center px-2.5 py-0.5 text-xs font-mono font-black bg-accent text-white shadow-xs">
                          ...
                          {category.badge && (
                            <span className="text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 bg-accent/10 text-accent">
                              {category.badge}
                            </span>
                          )}

                          {/* Page Range Badge */}
                          <span className="text-xs font-semibold text-slate-600 bg-white px-2.5 py-0.5 border border-slate-200 font-mono">
                    ...
                    <button 
                      className="text-slate-400 hover:text-slate-700 p-2 transition-colors shrink-0 ml-2 rounded-[50px]"
                  ...
                  {isExpanded && (
                    <div className="p-5 sm:p-8">
                      {viewMode === 'grid' ? (
                        ...
                        >
                          {category.enrichedLinks.map((link) => (
                            <li key={link.path + link.label + link.globalNumber}>
                              <Link
                                to={link.path}
                                className="group block p-4 rounded-[1px] border border-slate-100 hover:border-accent/40 bg-white hover:bg-slate-50/90 transition-all duration-150 h-full flex flex-col justify-between shadow-xs hover:shadow-sm rounded-[50px]"
                              >
                                <div>
                                  {/* Entry Identification Header: Hierarchy Code & Global Number */}
                                  <div className="flex items-center justify-between gap-2 mb-2">
                                    <div className="flex items-center gap-1.5">
                                      <span className="inline-flex items-center px-2 py-0.5 bg-slate-900 text-accent font-mono font-bold text-xs rounded-[50px]">
                                        {link.hierarchicalId}
                                      </span>
                        ...
                        <div className="overflow-x-auto rounded-[1px] border border-slate-200">
                              ...
                              {category.enrichedLinks.map((link) => (
                                ...
                                >
                                  <td className="py-3 px-3 text-center">
                                    <span className="inline-block px-2 py-0.5 bg-slate-900 text-accent font-mono font-bold text-xs rounded-[50px]">
                                      {link.hierarchicalId}
                                    </span>
                                  </td>
                                  ...
                                  <td className="py-3 px-3 text-right">
                                    <Link
                                      to={link.path}
                                      className="inline-flex items-center gap-1 px-3 py-1 text-xs font-bold bg-slate-100 group-hover:bg-accent group-hover:text-white text-slate-700 transition-all rounded-[50px]"
                                    >
                                      <span>Visit</span>
        ...
        <div className="mt-12 bg-white rounded-[1px] p-6 sm:p-8 border border-slate-200 text-center max-w-4xl mx-auto shadow-sm">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a 
              href="tel:0244753849" 
              className="inline-flex items-center gap-2 px-6 py-3 bg-accent text-white font-bold text-sm hover:bg-accent/90 transition-all shadow-md shadow-accent/20 rounded-[50px]"
            >
              <Truck className="w-4 h-4" />
            ...
            <Link 
              to="/contact" 
              className="inline-flex items-center gap-2 px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-sm transition-all rounded-[50px]"
            >
